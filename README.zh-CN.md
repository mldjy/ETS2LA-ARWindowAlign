...
**着色器 / 全画质**（「简化图形」= **关**，只补 `WorldToScreen` 会漏掉的那条通道）：

![着色器通道对齐](docs/verify-shader-path.png)

**ImGui 2D**（「简化图形」= **开**）：

![ImGui 通道对齐](docs/verify-imgui-path.png)

*两张均为 3840×2160 桌面、游戏窗口 (1907,524) 1920×1080 下实测：AR 路径带贴合车道并向消失点收束。*
